...
int realize(Input_struct* input_struct, Image_info* image_info, int* microstructure_id, const int my_rank);

// Function declaration for the auxiliary algorithm of traversing the seed connection graph
...
int main(int argc, char* argv[]) {
	...
	int line_number = 0; // The total number of lines in the input file
	int errnum; // Error number
	int return_value;
	double time_start;
	if (my_rank == 0) {
		time_start = MPI_Wtime();
		FILE* summary_file = fopen("summary_mpi.txt", "a");
	  	fprintf(summary_file, "%i\t", comm_sz);
		fclose(summary_file);
		/* Parsing the input file of the program */
		...
		// Sending the "line_number" variable to all MPI processors
		for (int i=1; i<comm_sz; i++) {
			MPI_Send(&line_number, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(buffer2, MAX_CHAR_PATH, MPI_CHAR, i, 0, MPI_COMM_WORLD);
		}

		// Create the input files for all MPI processors seperately (each tasked with a segment of the user input data)
		for (int i = 0; i < comm_sz; i++) {
			// printf("%d, %d, %d, %d\n", i, line_number, line_number%comm_sz, line_number/comm_sz);
			snprintf(buffer, MAX_CHAR_PATH, "%s/input_mpi_%02d.txt", buffer2, i);
			FILE* input_file_i = fopen(buffer, "w");
			if (i == 0)
				for (int j = 1; j <= line_number%comm_sz; j++) {
					fgets(buffer, MAX_CHAR_PATH, input_file);
					fprintf(input_file_i, "%s", buffer);
				}
			for (int j = i*(line_number/comm_sz)+1; j <= (i+1)*(line_number/comm_sz); j++) {
				fgets(buffer, MAX_CHAR_PATH, input_file);
				fprintf(input_file_i, "%s", buffer);
			}
			// fprintf(input_file_i, "\n");
			fclose(input_file_i);
		}
		fclose(input_file);
	...
	else { // If this code is run by an MPI processor other than the master with rank0
		// Recieving the "line_number" variable from the master processor (rank0)
		MPI_Recv(&line_number, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(buffer2, MAX_CHAR_PATH, MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	...
	// Read the input file specifically generated for the current processor
	snprintf(buffer, MAX_CHAR_PATH, "./%s/input_mpi_%02d.txt", buffer2, my_rank);
	FILE* input_file = fopen(buffer, "r");
	int iterations;
	if (my_rank == 0)
		iterations = line_number / comm_sz + line_number % comm_sz;
	else
		iterations = line_number / comm_sz;
	for (int i = 1; i <= iterations; i++) { // (int i = my_rank*(line_number/comm_sz)+1; i <= (my_rank+1)*(line_number/comm_sz); i++)
		fgets(buffer, MAX_CHAR_PATH, input_file); // Reading one complete line (including end of line character) from the file
		// Path to the results directory
		...
		realize(&input_struct, &image_info, &microstructure_id, my_rank);
	}
	delete[] buffer;
	delete[] buffer2;
	fclose(input_file);
...
int realize(Input_struct* input_struct, Image_info* image_info, int* microstructure_id, const int my_rank) {
	...
	snprintf(bbuffer, MAX_CHAR_PATH, "%s/log_thread_%02d.txt", input_struct->results_directory.c_str(), my_rank); // Creating a log file for each processing thread/instantiation of this function
// #pragma warning(suppress : 4996) // Similarly done for the whole enviroment by adding "_CRT_SECURE_NO_WARNING=1" to the preprocessor definitions  
	...
	start_time = MPI_Wtime(); // Start time of the current microstructure realization (thread-safe/private)
	// Seeding the RNG based on the time of code execution to get different random numbers in each execution
	// srand(time(NULL));
	// std::mt19937 mt(time(NULL));
	// std::mt19937 mt((((unsigned)time(NULL)) & 0xFFFFFFF0) | (my_rank + 1));
	std::mt19937 mt(((unsigned)time(NULL) & 0xFFFFFFF0) | (my_rank + 1));
...
// #pragma omp threadprivate(pixel_array)
	...
	std::vector<float> phase_volume_previous(number_of_phases - 1, 0);
	std::vector<float> phase_volume_min(number_of_phases - 1, 0.55);
	for (i = 0; i < number_of_phases - 1; i++) {
		if (input_struct->volume_fraction[i] < 0.55)
			phase_volume_min[i] = 0.0;
		else
			phase_volume_min[i] = 0.55;
	}
	std::vector<float> phase_volume_max(number_of_phases - 1, 0.95);
	for (i = 0; i < number_of_phases - 1; i++) {
		if (input_struct->volume_fraction[i] > 0.95)
			phase_volume_max[i] = 1.0;
		else
			phase_volume_max[i] = 0.95;
	}
	...
	std::vector<int> phase_volume_copy(number_of_phases - 1, 0); // Initialization of each phase volume in terms of pixels/voxels
	...
	start_time_trial = MPI_Wtime(); // Start time of the current microstructure realization (thread-safe/private)
	// Initialization
	std::mt19937 mt(((unsigned)time(NULL) & 0xFFFFFFF0) | (my_rank + 1));
	...
	while (1 > 0) {
		...
		end_time = MPI_Wtime(); // End time of the current microstructure realization (thread-safe/private)
	...
	for (phase = 1; !gotoMainLoop && phase < number_of_phases; phase++) {
		...
		if ((double)phase_volume[phase - 1] / (double)loop_threshold > phase_volume_max[phase - 1] || phase_volume[phase - 1] > phase_volume_threshold[phase - 1] + (int)(loop_threshold * 0.05)) { // phase_volume[phase - 1] > phase_volume_threshold[phase - 1] + (int)(loop_threshold * 0.05) ||
			fprintf(log_file, "%06d- Phase %2d: Realization VF = %.3f > Max Volume Fraction(VF) = %.3f OR Realization VF = %.3f > Volume Fraction(VF) = %.3f + 0.05 -> The realization VF is too high, so no outputs will be generated due to failure.\n", *microstructure_id, phase, ((double)phase_volume[phase - 1] / (double)loop_threshold), phase_volume_max[phase - 1], ((double)phase_volume[phase - 1] / (double)loop_threshold), input_struct->volume_fraction[phase - 1]);
			fflush(log_file); // Flush out the file stream so that the user can see what new things has been written to the log file during program execution
			break;
		}
		if (phase_volume[phase - 1] < phase_volume_threshold[phase - 1] - (int)(loop_threshold * 0.35)) { // && input_struct->neighborhood_radius[phase - 1] < 4
			fprintf(log_file, "%06d- Phase %2d: Realization VF = %.3f < Volume Fraction(VF) = %.3f - 0.35 -> The realization VF is too low, so no outputs will be generated due to failure.\n", *microstructure_id, phase, ((double)phase_volume[phase - 1] / (double)loop_threshold), input_struct->volume_fraction[phase - 1]);
			fflush(log_file); // Flush out the file stream so that the user can see what new things has been written to the log file during program execution
			break;
		}
		...
		while (!clustered) { // && (phase_volume[phase - 1] < phase_volume_threshold[phase - 1])
			...
			if (connection_array[max_index].size() && ((float)max / (float)(phase_volume[phase - 1]) > 0.85))
				clustered = true;
		...
		// Checking whether the current volume fraction falls into an acceptable range around the target volume fraction
		if ((double)phase_volume[phase - 1] / (double)loop_threshold > phase_volume_max[phase - 1] || phase_volume[phase - 1] > phase_volume_threshold[phase - 1] + (int)(loop_threshold * 0.05)) { // phase_volume[phase - 1] > phase_volume_threshold[phase - 1] + (int)(loop_threshold * 0.05) ||
			fprintf(log_file, "%06d- Phase %2d: Realization VF = %.3f > Volume Fraction(VF) = %.3f + 0.05 -> The realization VF is too high, so no outputs will be generated due to failure.\n", *microstructure_id, phase, ((double)phase_volume[phase - 1] / (double)loop_threshold), input_struct->volume_fraction[phase - 1]);
			fflush(log_file); // Flush out the file stream so that the user can see what new things has been written to the log file during program execution
			break;
		}
...
// Get and preprocess the input of the main function (Not used for now!)
void get_input(int argc, char* argv[], const int my_rank, const int comm_sz, int* N) {
  	if (my_rank==0) {
